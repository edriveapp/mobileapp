...
export class PaymentsService {
  ...
  private parseMetadata(metadata: any) {
    const customFields = Array.isArray(metadata?.custom_fields)
      ...
      : [];
    const customMap = customFields.reduce(
    ...
    return {
      ...
      distanceInKm: Number(
        metadata?.distanceInKm ?? customMap?.distance_km ?? 0,
      ),
      estimatedDurationMinutes: Number(
        metadata?.estimatedDurationMinutes ??
          customMap?.estimated_duration_minutes ??
          0,
      ),
    };
  }
  ...
  private async processPaystackSuccessPayload(payload: any): Promise<void> {
    ...
    if (
      await this.ridesService.isPaymentReferenceProcessed(payload?.reference)
    ) {
      this.logger.log(
        `Paystack transaction already processed: ${payload?.reference}`,
      );
      return;
    }
    ...
    if (isRemittance && remittanceUserId) {
      await this.ridesService.processRemittancePayment(
        remittanceUserId,
        amount,
        payload?.reference,
      );
      this.logger.log(
        `Remittance payment confirmed: userId=${remittanceUserId} amount=₦${amount}`,
      );
      return;
    }
    ...
    if (!rideId) {
      this.logger.warn(
        `Paystack success payload missing rideId: ref=${payload?.reference}`,
      );
      return;
    }
    ...
    if (userId) {
      const bookedRide = await this.ridesService.bookPublishedTrip(
        rideId,
        userId,
        {
          paymentMethod: "card",
  ...
  async calculatePaymentSplit(
    ...
    try {
      const settingsUser = await this.usersRepository.findOne({
    ...
    const platformCutAmount = Number(
      ((amount * platformCutPercent) / 100).toFixed(2),
    );
    const insuranceReserveAmount = Number(
      ((amount * insuranceReservePercent) / 100).toFixed(2),
    );
    const driverNetEarnings = Number(
      (amount - platformCutAmount - insuranceReserveAmount).toFixed(2),
    );
  ...
  async initializePayment(
    ...
    if (ride.passengerId === userId || ride.driverId === userId) {
      ...
      const booking = await this.bookingsRepository.findOne({
        where: { rideId, passengerId: userId, status: BookingStatus.CONFIRMED },
      });
      if (booking) {
      ...
      metadata: {
        ...
        custom_fields: [
          ...
          { display_name: "User ID", variable_name: "user_id", value: userId },
          {
            ...
            variable_name: "distance_km",
            value: distanceInKm,
          },
          {
            display_name: "Estimated Duration (minutes)",
            variable_name: "estimated_duration_minutes",
            value: estimatedDurationMinutes,
          },
          {
            display_name: "Driver Net Earnings",
            variable_name: "driver_net_earnings",
            value: split.driverNetEarnings,
          },
          {
            display_name: "Platform Fee",
            variable_name: "platform_fee",
            value: split.platformCutAmount,
          },
          {
            display_name: "Insurance Reserve",
            variable_name: "insurance_reserve",
            value: split.insuranceReserveAmount,
          },
        ],
      },
    };
  ...
  async initializeRemittance(userId: string, email: string) {
    ...
    try {
      ...
      this.logger.error(
        "Remittance initialization failed:",
        error.response?.data || error.message,
      );
      throw new InternalServerErrorException(
        "Remittance initialization failed",
  ...
  async initiateRefund(
    ...
    reason: string,
    amountNaira?: number,
  ): Promise<{ refundReference: string; amount: number }> {
    const secretKey = this.configService.get<string>("PAYSTACK_SECRET_KEY");
    if (!secretKey)
      throw new InternalServerErrorException(
        "PAYSTACK_SECRET_KEY is not configured",
    ...
    const ride = await this.ridesService.findRideById(rideId);
    if (!ride) throw new NotFoundException("Ride not found");
    if (!ride.paystackReference)
      throw new BadRequestException(
        "No payment reference found for this ride — cash rides cannot be refunded via Paystack",
      );
    if (ride.paymentStatus === "refunded")
      throw new BadRequestException("This ride has already been refunded");
    if (ride.paymentStatus !== "paid")
      throw new BadRequestException("Only paid rides can be refunded");

    const refundAmountKobo = amountNaira
    ...
    try {
      ...
      const refundReference =
        refundData?.id?.toString() || `refund_${Date.now()}`;
      const refundedAmount =
        (refundData?.amount ??
          refundAmountKobo ??
          Number(ride.tripFare) * 100) / 100;

      await this.ridesService.markRefunded(rideId, refundReference, reason);
      this.logger.log(
        `Refund initiated for ride ${rideId}: ref=${refundReference} amount=₦${refundedAmount}`,
      );
  ...
  async getBanks() {
    ...
    try {
      ...
      this.logger.error(
        `Bank list fetch failed: ${error.response?.data?.message || error.message}`,
      );
  ...
  async resolveAccountNumber(accountNumber: string, bankCode: string) {
    ...
    try {
      ...
      const message =
        error.response?.data?.message || "Could not resolve account number";
