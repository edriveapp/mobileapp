...
export class MailerService {
  ...
  getDefaultFromEmail() {
    return (
      this.configService.get<string>("EMAIL_FROM") || "support@edriveapp.com"
    );
  }
...
<body style="margin: 0; padding: 20px; background: #f9f9f9;">
  ...
  async sendEmail(
    ...
    const recipients = Array.from(
      new Set((to || []).map((v) => v?.trim()).filter(Boolean)),
    );
    if (!recipients.length) return { sent: false, reason: "no_recipients" };

    const provider = (
      this.configService.get<string>("EMAIL_PROVIDER") || "none"
    ).toLowerCase();
    const from = this.formatFromAddress(
      options?.from?.trim() || this.getDefaultFromEmail(),
    );
  ...
  async sendBatch(
    ...
    if (provider !== "resend") {
      this.logger.log(
        `Batch provider not configured. Would send ${messages.length} emails.`,
      );
      return { sent: false, reason: "provider_not_configured" };
    }
    ...
    try {
      const chunkSize = messages.some((m) => m.attachments?.length) ? 1 : 100;
      for (const chunk of this.chunkArray(messages, chunkSize)) {
  ...
  private async sendWithResend(
    ...
    try {
      ...
      if (!response.ok) {
        const body = await response.text();
        this.logger.warn(
          `Resend batch send failed: ${response.status} ${body}`,
        );
        return { sent: false, reason: `resend_${response.status}` };
      }
